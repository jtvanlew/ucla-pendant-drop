...
#x = 5*cos(R) + 0.2*random.rand(len(R))
#y = 5*sin(R) + 0.2*random.rand(len(R))
...
def plot_all(residu2=False):
    ...
    plt.plot(x_fit1, y_fit1, 'b-' , label=method_1, lw=2)
    ...
    plt.plot(x_fit3, y_fit3, 'r-.', label=method_3, lw=2)

    plt.plot([xc_1], [yc_1], 'bD', mec='y', mew=1)
    plt.plot([xc_2], [yc_2], 'gD', mec='r', mew=1)
    plt.plot([xc_3], [yc_3], 'kD', mec='w', mew=1)
    ...
    plt.savefig('output/%s_residu%d.png' % (basename, 2 if residu2 else 1))
